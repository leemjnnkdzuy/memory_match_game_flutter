...
const initializeSocketHandlers = (io) => {
	io.on("connection", (socket) => {
			...
			try {
				...
				if (result) {
					...
					socket.emit("solo_duel:queue_joined", {
						position: matchmakingService.getQueueInfo().queueLength,
					});
				}
			} catch (error) {
		...
		socket.on("disconnect", () => {
			...
			if (socket.matchId) {
				io.to(socket.matchId).emit("solo_duel:player_disconnected", {
					userId: socket.userId,
					username: socket.username,
				});
			}
...
const saveMatchHistory = async (match) => {
	const gameTime = Math.floor((match.finishedAt - match.startedAt) / 1000);
